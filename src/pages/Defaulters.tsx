...
import { fetchEpiSchedule, saveClient } from "@/queries/clients";
...
const Defaulters = () => {
  ...
  const { role, accountId, facilityId, profile } = useAuth();
  ...
  const saveClientMutation = useMutation({
    mutationFn: (variables: { data: ClientFormValues; clientId?: string }) => {
      const assignedTo = profile?.first_name
        ...
        : 'System';
      return saveClient({ ...variables, epiSchedule, accountId: accountId!, facilityId: facilityId!, assignedTo });
    },
    onSuccess: () => {
  ...
  const runCheckMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.rpc("auto_detect_defaulters");
  ...
  const resyncMutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.rpc("auto_resync_clients");
      if (error) throw error;
      return data as number;
    },
    onSuccess: (count) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <AddClientDialog
        ...
        onSave={(data, clientId) => saveClientMutation.mutate({ data, clientId })}
      />
    </div>
  );
};
